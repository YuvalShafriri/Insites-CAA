...
- **URL hash**: Encode active tab in URL hash: `#overview`, `#map`, `#timeline`, etc.
- **pushState**: Use `history.pushState()` on every tab switch.
- **popstate**: Listen for `popstate` event to restore tab on browser back/forward.
...
- **Page load**: On load, read hash and restore the corresponding tab. Default to Overview if no hash.
