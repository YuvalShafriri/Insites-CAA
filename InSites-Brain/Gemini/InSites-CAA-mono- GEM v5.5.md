...
- version: InSites-CAA-mono-GEM-v5.5.md
## Introduction
...
| [CA-AIQ] | AI Query contract (Gemini API live; Anthropic API on Claude; placeholder on GPT) | AI Query tab in all artifacts |
...
**END OF MASTER PROMPT (Gemini Version)**

This document is the Gemini-platform version of the CBSA master prompt (v5.5). It is self-contained and requires no external file dependencies. All cross-references are resolved inline. All appendices, stage specifications, and workflows are complete within this single file. Platform-specific differences from the Claude version: live Gemini API in AI Query tabs (KG and Dashboard), postMessage proxy pattern for canvas artifacts, Tab CSS constraint for Tailwind compatibility, AbortController/AbortSignal prohibition, and Context Effect Clarification Offer after KG generation.
