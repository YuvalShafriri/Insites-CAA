...
Generate an interactive Knowledge Graph HTML artifact when the user explicitly requests a Knowledge Graph ("kg", "knowledge graph", "create kg").
...
- CRITICAL: Do NOT use `AbortController` or `AbortSignal` for fetch timeout — they cannot be cloned through artifact sandbox `postMessage` proxy (`DataCloneError`). Use `Promise.race` with `setTimeout` for timeout instead.
...
- **No ESM imports**: Do NOT use `import` statements for CDN libraries — the canvas sandbox does not support dynamic `require()`. Load all libraries via `<script>` tags and access via global objects (`window.d3`, `window.L`, `window.Chart`).
...
15. **Tab CSS constraint**: `.tab-content:not(.active) { display: none !important; }` present in `<style>` block.

16. **No ESM imports**: No `import` statements for CDN libraries. All loaded via `<script>` tags, accessed via `window.*` globals.

17. **AI Query tab** implements [CA-AIQ] contract with correct platform mode (Gemini API live).
...
| Comparative references — what compared to, and on what basis | `—` |

| Threats | `—` |

| Assessment method | `—` |

| Value specifications — for each value, what it specifically means at *this* site | `⚠ not specified` |
...
- If location information includes geographic references, attempt approximate coordinates.
...
Common analysis types (offer when relevant):

- **Thematic classification** — group by significance type, heritage character, or emergent categories. Multiple overlapping schemes.
...
- **Enrichment needs** — derivable dimensions vs. those needing additional data.
...
If too thin for even a Profile: state what's present, what's needed, offer options.
...
> **Scope**: Collection-level visualization (multiple sites from MA-RC analysis). For single-assessment dashboards (one site, one CBSA process), see [CA-DB] above. Both share the same UX foundation (guide boxes, inline data, CDN loading, tab CSS constraint) but have different data shapes, tab structures, and visual palettes. Collection: Inter + stone/amber palette.
...
| Values identified | values: { [type]: "e"/"i"/"a" } | Map to 8 categories. e = explicit, i = implied, a = absent |
...
| 1 | **Overview** | KPI cards + 4 distribution charts | Always first tab. Orients the user. |

| 2 | **Map** | Leaflet map with circle markers | Filter buttons per value type. Click marker → popup. |

| 3 | **Values** | Matrix: sites × value types, ●/◐/○ | Sortable columns. Sticky first column. Footer counts. |

| 4 | **Arguments** | Significance premises + claim scope | Table with strength color-coding. |

| 5 | **Gaps** | Traffic-light matrix: sites × data dimensions | Per-site completeness score. |

| 6 | **Cross-Tabs** | Stacked bar charts | Show ALL categories — no silent truncation. |

| 7 | **Clusters** | Management-oriented grouping cards | Sites may appear in multiple clusters. |

| 8 | **AI Query** | In-artifact heritage analysis chat | Implements [CA-AIQ] contract. Gemini: Gemini API. See §9 below. |
...
- **Cross-tab site linking.** All site names clickable → navigate to Map popup or Values row.

- **No silent truncation.** Charts must show all data categories.

- **Guide boxes.** Each tab gets a collapsible guide box (collapsed by default; header = tab purpose in one sentence; body = 2–3 bullet interpretation hints).
...
- **Tab CSS constraint.** Tailwind's `flex` utilities override display hiding. Include: `.tab-content:not(.active) { display: none !important; }`

- **No ESM imports.** Do NOT use `import` statements for CDN libraries — the canvas sandbox does not support dynamic `require()`. Load all libraries via `<script>` tags and access via global objects (`window.Chart`, `window.L`).
...
| [CA-AIQ] | AI Query contract (Anthropic / Gemini / placeholder) | AI Query tab in all artifacts |
